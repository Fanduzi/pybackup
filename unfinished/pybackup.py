...
"""
Usage:
        pybackup.py mydumper ARG_WITHOUT_--... (([--no-rsync] [--no-history]) | [--only-backup])
...
def confLog():
    ...
    if arguments['only-rsync'] or arguments['validate-backup']:
        ...
        log_file = [x for x in arguments['ARG_WITHOUT_--'] if 'logfile' in x]
        if not log_file:
            print('必须指定--logfile选项')
            sys.exit(1)
        else:
            log = log_file[0].split('=')[1]
            arguments['ARG_WITHOUT_--'].remove(log_file[0])
...
def getOption(cmd,option):
    cmd_list = cmd.split(' ')
    return [ x.split('=')[1] for x in cmd_list if option in x ][0]
...
def runBackup(targetdb):
    '''执行备份'''
    # 是否指定了--database参数
    isDatabase = [ x for x in arguments['ARG_WITHOUT_--'] if 'database' in x ]
    isTables_list = [ x for x in arguments['ARG_WITHOUT_--'] if 'tables-list' in x ]
    isRegex = [ x for x in arguments['ARG_WITHOUT_--'] if 'regex' in x ]
    # 备份的数据库 字符串
    ...
    if isTables_list:
        ...
        #密码中可能有带'#'或括号的,处理一下用引号包起来
        enclosePass(cmd)
        backup_dest = getOption(cmd,'outputdir')
        if backup_dest[-1] != '/':
    ...
    # 如果指定了--regex参数
        ...
        # 密码中可能有带'#'或括号的,处理一下用引号包起来
        enclosePass(cmd)
        regex_expression = getOption(cmd,'regex')
    ...
    # 没有指定--database参数
        ...
        if db_consistency.upper() == 'TRUE':
            # 如果在pybackup.conf中设置db_consistency为true,则使用--regex来备份, 保证一致性
            ...
            # 检查备份是否成功
            end_time = judgeStatus(state)
                # --regex的备份都在同一个目录下, 这里按db创建目录, 将备份文件移动到对应的文件夹中, 再将metedata拷贝进去
                for db in bdb_list:
            ...
            # 否则, 则多次执行mydumper创建多个备份, 每个备份都要有成功与否状态标记
            is_complete = ''
            # 在备份列表中循环
            for i in bdb_list:
                print(i)
...
def safeCommand(cmd):
    '''移除user_backup表中, bk_command列中的密码'''
    cmd_list = cmd.split(' ')
    passwd = getOption(cmd,'password')
        ...
        if result:
            ...
            if dbs:
                for db in dbs:
                    ...
                    1 个 bk_id 对应3个备份,1 个 bk_id 对应1个备份 ,但是tag只append 了俩, 应该每个库append一次,或者改成字典
                    '''
                    tags.append(tag)
...
if __name__ == '__main__':
    ...
    if arguments['mydumper'] and ('help' in arguments['ARG_WITHOUT_--'][0]):
...
#                print(sql1 % (tag.decode('utf-8'), bk_id, backup_path, db, stime, etime, (etime - stime).total_seconds(), status))
        ...
        if arguments['mydumper']:
            mydumper_args = ['--' + x for x in arguments['ARG_WITHOUT_--']]
            is_rsync = True
            is_history = True
        ...
        bk_dir = [x for x in arguments['ARG_WITHOUT_--'] if 'outputdir' in x][0].split('=')[1]
        os.chdir(bk_dir)
